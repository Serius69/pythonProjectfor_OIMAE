...
# Cambia 'ruta_del_archivo.xlsx' por la ruta del archivo que quieras usar
df = pd.read_excel('ruta_del_archivo.xlsx', index_col=0)

# Graficar el heatmap de sensibilidad (VAN)
plt.figure(figsize=(12, 8))
sns.heatmap(df, annot=True, cmap="RdYlGn", cbar_kws={'label': 'Diferencia respecto a la ganancia bruta actual [$ Millones USD]'})
...
sensitivity_df = pd.DataFrame({
    ...
    "Precio boleto [$USD]": np.tile(df.columns[1:], len(df.index) - 1),  # Excluir primera columna
